...
const EditOrder = ({id ,ordersDetails}) =>{
    ...
    useEffect(() =>{
        setData({...data , 
            id: ordersDetails.data_1.owner.id,
        ...
        <div className="card">
            ...
            <div className='forms-sample'>
                <div className={styles.container}>
                    ...
                    <input  disabled type="text" className="form-control form-control-lg" id="priceTotalInput" placeholder=" کد پستی" value={data.total_price} />
                    </div>
                    <div className="form-group">
                    <label htmlFor="confirmationInput">وضعیت پرداخت</label>
                    <input  disabled type="text"  className="form-control form-control-lg" id="confirmationInput" placeholder=" کد پستی" value={data.payment_status == 'p' ? 'پرداخت شده' : 'مرجوعی'} />
                    </div>
                    <div className="form-group">
                    <label htmlFor="dateInput">تاریخ</label>
                    <input  disabled type="text"  className="form-control form-control-lg" id="dateInput" placeholder=" کد پستی" value={shamsi.gregorianToJalali(data.payment_date.split("-")).join("-")} />
                    </div>
                </div>
                <h4 className="card-title">محصولات خرید شده</h4>
                    {ordersDetails.data_2.map(item =>
                         ...
                         <div key={item.id} className="form-group">
                            ...
                            <input  disabled type="text"  className="form-control form-control-lg" id="nameInput" placeholder="نام کاربری" value={item.product.name} />
                         </div>
                         <div className="form-group">
                            <label htmlFor="priceInput">قیمت محصول </label>
                            <input  disabled type="text"  className="form-control form-control-lg" id="priceInput" placeholder="ایمیل" value={item.product_price} />
                         </div>
                         <div className="form-group">
                            <label htmlFor="quantityInput">تعداد انتخاب شده </label>
                            <input  disabled type="text" className="form-control form-control-lg" id="quantityInput" placeholder="نام "  value={item.quantity}/>
                         </div>
                         <hr />
                         </>
                        )}
